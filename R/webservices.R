...
#'      \code{irisws.<network>.<station>.<location>.<channel>.<starttime>.<end>.<extension>}
...
ws.timeseries <- function(network, station, location, channel, 
    ...
    if (outp=="sac.bin"){
        ...
        if (verbose) message(paste(wt,"sac binary:", endi, "endian"))
        # needs 'sacbl' or 'sacbb'
        outp <- paste0("sacb",substr(endi,1,1))
    } else if (outp=="sac.asc"){
        outp <- 'saca'
        if (verbose) message(paste(wt, "sac ascii"))
    } else if (outp=="ascii.values"){
        outp <- 'ascii1'
        if (verbose) message(paste(wt, "ascii (values only)"))
    } else {
        if (verbose) message(paste(wt, outp))
    }
    ...
    if (!is.null(filename)){
        if (is.na(filename)) {
            ...
            filename <- paste("irisws", network, station, location, channel, starttime, fiendtime, ext, sep=".")
        }
    ...
    #
    toret <- list(query=Q, querydata=list(), opts=opts)
